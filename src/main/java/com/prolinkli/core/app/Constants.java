...
public final class Constants {
  ...
  public static final class User {
	 ...
	 * 
	 * These constants define the supported authentication methods in the system:
	 * - PASSWORD: Traditional username/password authentication
	 * - GOOGLE_OAUTH2: Google OAuth2 authentication
	 * - FACEBOOK_OAUTH2: Facebook OAuth2 authentication (planned)
	 * - MICROSOFT_OAUTH2: Microsoft OAuth2 authentication (planned)
	 * 
	 * Values correspond to entries in the lk_user_authentication_methods database table
	 * and are used throughout the authentication system for method identification.
	 */
   ...
   * - OAuth2Controller URL path variables (/auth/oauth2/{provider})
   * - Service registration and lookup in OAuth2Controller
   * - Frontend OAuth2 provider selection
   * 
   * Must match the provider names returned by AbstractOAuthService.getProviderName()
   ...
   * @documentation-pr-rule.mdc
   * 
   * Contains nested classes for each OAuth2 provider with their specific
   * parameter keys used in callback handling and token exchange.
   * 
   * These constants ensure consistent parameter handling across OAuth2 flows
  ...
  public static final class OAuth2Providers {
     ...
     * @documentation-pr-rule.mdc
     * 
     * - CODE_KEY: Authorization code parameter from Google callback
     * - STATE_KEY: CSRF protection state parameter from Google callback
     * 
     * Used in GoogleOAuthService for processing OAuth2 callback parameters
  ...
  public static final class Jwt {
	 ...
	 * @documentation-pr-rule.mdc
	 * 
	 * Contains nested classes for each authentication method with their specific
	 * parameter keys used in credential validation and processing.
	 * 
	 * These constants ensure type safety and prevent parameter key mismatches
	...
	public static final class AuthenticationKeys {
		 ...
		 * @documentation-pr-rule.mdc
		 * 
		 * Parameter keys used in Google OAuth2 authentication flow:
		 * - CODE: Authorization code from OAuth2 callback
		 * - STATE: CSRF protection state parameter
		 * - SUBJECT: Google user ID (sub claim) from ID token
		 * - ID_TOKEN: Google ID token containing user profile
		 * - ACCESS_TOKEN: Google access token for API calls
		 * 
		 * Used by GoogleOAuth2Provider for credential validation and
